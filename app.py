...
if __name__ == '__main__':
  ...
  superTienda_raw['Anho']= pd.to_datetime(superTienda_raw['Fecha_pedido']).dt.year.astype(int)
  superTienda_raw['Mes']= pd.to_datetime(superTienda_raw['Fecha_pedido']).dt.month.astype(int)
  ...
  with st.container():
      ...
      with line_chart_total:
          # grafico de lineas
          data_line = superTienda_raw_filter.groupby('Fecha_pedido')['Ventas'].sum().reset_index()
      ...
      with pie_chart_total:
          ...
          data_pie = superTienda_raw_filter.groupby('País/Región')['Ventas'].sum().reset_index()
  ...
  with st.container():
      ...
      with bar_chart_total:
          ...
          data_bar = superTienda_raw_filter.groupby('Categoría')['Ventas'].sum().reset_index()
      ...
      with bar_chart_total2:
          ...
          data_bar2 = superTienda_raw_filter.groupby('Subcategoría')['Ventas'].sum().reset_index()
